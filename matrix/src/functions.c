...
/*---------------------------------------------------------------------------*/
void* simpleMatrixProizvAsmp(void * args){
...
//  p->thr[0]--;
  ...
  if (p->cursorlevel == 1){
    while (threadn > 0){ printf("do nothing %d\n", threadn); cntr++; sleep(2);}
  }
  return NULL;
}
